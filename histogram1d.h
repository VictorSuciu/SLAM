...
class Histogram1D {
private:
    float minValue; // the minimum value the histogram is allowed to count
    float maxValue; // the maximum value the histogram is allowed to count
    float binSize; // the range of each bin
...
public:
    ...
    Histogram1D(float minValue, float maxValue, float binSize);
