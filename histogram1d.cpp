 ...
 */
Histogram1D::Histogram1D(float minValue, float maxValue, float binSize) {
 ...
 */
bool Histogram1D::insert(float val) {
    if(withinRange(val)) {
 ...
 */
bool Histogram1D::isMaxBin(float val) {
    if(!maxIsUpToDate) {
        computeMaxBin();
    }
 ...
 * val: the value to test
 */
bool Histogram1D::withinRange(float val) {
